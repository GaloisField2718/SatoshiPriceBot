...
async def btc2usd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info("btc2usd called \n")        
    try:
        user_input = context.args[0] if context.args else None
        if user_input is not None:
            amount_input = float(format_number_with_dot(user_input))
            res = convert.btcusd(amount_input, 0)
            formatted_input = "{:,.8f}".format(amount_input)
            formatted_res = format_number_with_dot("{:.2f}".format(res))
            message = f"{formatted_input} BTC = `{formatted_res}` USD"
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=message,
                parse_mode='Markdown'
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Please enter the amount of Bitcoin you want to convert to Dollars! Use /btc2usd amount."
            )
    except Exception as e:
        logging.error(f"Error during btc2usd conversion: {e} in {update}\n")
        message_error = f"Error during conversion: {e}"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=message_error)
...
async def usd2btc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info("usd2btc called \n")
    try:
        user_input = context.args[0] if context.args else None
        if user_input is not None:
            amount_input = float(format_number_with_dot(user_input))
            res = convert.btcusd(amount_input, 1)
            formatted_input = format_number_with_dot("{:.2f}".format(amount_input))
            formatted_res = format_number_with_dot("{:.8f}".format(res))
            message = f"{formatted_input} USD = `{formatted_res}` BTC"
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=message,
                parse_mode='Markdown'
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Please enter the amount in Dollars you want to convert to Bitcoin! Use /usd2btc amount."
            )
    except Exception as e:
        logging.error(f"Error during usd2btc conversion: {e} in {update}\n")
        message_error = f"Error during conversion: {e}"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=message_error)
...
async def sat2usd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info("sat2usd called \n")
    try:
        user_input = context.args[0] if context.args else None
        if user_input is not None:
            amount_input = float(format_number_with_dot(user_input))
            res = convert.satsusd(int(amount_input), 0)
            formatted_input = format_number_with_dot(str(int(amount_input)))
            formatted_res = format_number_with_dot("{:.2f}".format(res))
            message = f"{formatted_input} sats = `{formatted_res}` USD"
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=message,
                parse_mode='Markdown'
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Please enter the amount of satoshis you want to convert to Dollars! Use /sat2usd amount."
            )
    except Exception as e:
        logging.error(f"Error during sat2usd conversion: {e} in {update}\n")
        message_error = f"Error during conversion: {e}"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=message_error)
...
async def usd2sat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info("usd2sat called \n")
    try:
        user_input = context.args[0] if context.args else None
        if user_input is not None:
            amount_input = float(format_number_with_dot(user_input))
            res = convert.satsusd(amount_input, 1)
            formatted_input = format_number_with_dot("{:.2f}".format(amount_input))
            formatted_res = format_number_with_dot(str(int(res)))
            message = f"{formatted_input} USD = `{formatted_res}` sats"
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=message,
                parse_mode='Markdown'
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="Please enter the amount in Dollars you want to convert to satoshis! Use /usd2sat amount."
            )
    except Exception as e:
        logging.error(f"Error during usd2sat conversion: {e} in {update}\n")
        message_error = f"Error during conversion: {e}"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=message_error)
...
async def inline_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conversion_prompts = {
        ...
        "convert_btc2usd": "Enter the amount of BTC to convert to USD:",

        "convert_eur2btc": "Enter the amount of EUR to convert to BTC:",
        "convert_usd2btc": "Enter the amount of USD to convert to BTC:",
        ...
        "convert_sat2usd": "Enter the amount of Sats to convert to USD:",

        "convert_eur2sat": "Enter the amount of EUR to convert to Sats:",  
        "convert_usd2sat": "Enter the amount of USD to convert to Sats:",  
    }
    ...
    try:
        ...
        if conversion_type:
            try:
                ...
                elif conversion_type == "convert_btc2usd":   
                    result = convert.btcusd(amount, 0)
                    formatted_result = format_number_with_dot("{:.2f}".format(result))
                    formatted_amount = format_number_with_dot("{:.8f}".format(amount))
                    message = f"{formatted_amount} BTC = `{formatted_result}` USD"
                    await update.message.reply_text(
                        text=message,
                        parse_mode='Markdown'
                    )
                ...
                elif conversion_type == "convert_sat2usd":
                    result = convert.satsusd(int(amount), 0)
                    formatted_result = format_number_with_dot("{:.2f}".format(result))
                    formatted_amount = format_number_with_dot(str(int(amount)))
                    message = f"{formatted_amount} Sats = `{formatted_result}` USD"
                    await update.message.reply_text(
                        text=message,
                        parse_mode='Markdown'
                    )
